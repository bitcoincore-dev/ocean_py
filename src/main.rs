...
#[clap(
    ...
    version,
    about,
    long_about = "Estimate cumulative loss for mining pools using Ocean's rules. Note: '丰' is the Unicode symbol for Satoshis."
)]
struct Args {
    ...
    /// Comma-separated list of other mining pool slugs to analyze (e.g.,
    /// antpool,f2pool)
    #[clap(long)]
    other_pools: Option<String>,

    /// Number of historical blocks to fetch and analyze for each pool. Analyzes
    ...
    /// Force an update of prices.json from mempool.space, regardless of whether
    /// it exists.
    #[clap(long)]
    update: bool,
}
...
async fn analyze_pool_loss(
    ...
    let base_blocks_url = format!(
        "https://mempool.space/api/v1/mining/pool/{}/blocks",
        pool_slug
    );
    ...
    if args.verbose {
        println!(
            "--- Starting Full History Crawl for {} ---",
            pool_slug.to_uppercase()
        );
    }
    ...
    let mut sorted_timestamps_indexed: Vec<(usize, i64)> =
        sorted_timestamps.into_iter().enumerate().collect();
    sorted_timestamps_indexed.sort_by_key(|&(_, ts)| ts);

    for b in all_blocks {
        let extras = b.extras.unwrap_or(PoolBlockExtras {
            match_rate: Some(0.0),
            reward: Some(0),
        });
        ...
        if args.verbose {
            println!(
                "DEBUG: Block height: {}, Raw timestamp: {}, Query timestamp: {}",
                b.height, b.timestamp, timestamp
            );
        }
...
fn print_ocean_health_table(ocean_slug: &str, ocean_processed_data: &[ProcessedBlockData]) {
    println!(
        "
--- OCEAN Health Report for {} ---",
        ocean_slug.to_uppercase()
    );
    println!(
        "{:<8} | {:<10} | {:<11} | {:<11} | {:<10} | {:<11} | {:<12} | {:<10}",
        "Height",
        "Time(UTC)",
        "Exp. Reward",
        "Reward($)",
    ...
    for block in ocean_processed_data {
        println!(
            "{:<8} | {:<10} | {:<11} | {:<11} | {:<10.2} | {:<11} | {:<12.2} | {:<10.2}",
            block.height,
            block.timestamp,
            block.expected_reward,
            block.actual_reward,
            block.health,
            block.loss_sats,
            block.loss_usd,
...
Analyzing {}...",
    ...
    println!(
        "TOTAL CUMULATIVE LOSS for {}: ${:.2}",
        ocean_slug.to_uppercase(),
        ocean_total_loss_usd
    );
    ...
    for other_pool_slug in other_pool_slugs {
        if other_pool_slug == ocean_slug {
            println!(
                "
Skipping loss estimation for {} as it is the reference pool.",
                ocean_slug.to_uppercase()
            );
            continue;
        }
...
",
            other_pool_slug.to_uppercase(),
            actual_ocean_depth
        );

        let other_pool_processed_data = analyze_pool_loss(
            other_pool_slug,
            Some(actual_ocean_depth),
            args,
            price_lookup_map,
        )
        .await?;

        let mut other_pool_data_by_timestamp: Vec<(usize, &ProcessedBlockData)> =
            other_pool_processed_data.iter().enumerate().collect();
        ...
        for ocean_block in &ocean_processed_data {
            if ocean_block.loss_sats > 0 && ocean_block.expected_reward > 0 {
                let ocean_loss_quotient =
                    ocean_block.loss_sats as f64 / ocean_block.expected_reward as f64;
                ...
                for (other_block_idx, other_block) in
                    other_pool_data_by_timestamp.iter().enumerate()
                {
                    ...
                    // Re-enabling the optimization with a check to ensure at least one block was
                    // found
                    if other_block_timestamp > ocean_block_timestamp + TIME_DIFFERENCE_THRESHOLD
                        && min_time_diff != i64::MAX
                    {
                        break;
                    }
                }
                ...
                if let Some(closest_other_block) = closest_other_block {
                    if closest_other_block_index != -1 && min_time_diff <= TIME_DIFFERENCE_THRESHOLD
                    {
                        ...
                        let other_pool_estimated_loss_sats =
                            ocean_loss_quotient * closest_other_block.actual_reward as f64;
                        let other_pool_estimated_loss_usd = (other_pool_estimated_loss_sats
                            / 100_000_000.0)
                            * closest_other_block.btc_usd;
                        estimated_other_pool_loss_usd += other_pool_estimated_loss_usd;
                        comparisons_made += 1;

                        let ocean_actual_usd = (ocean_block.actual_reward as f64 / 100_000_000.0)
                            * ocean_block.btc_usd;
                        let other_pool_actual_usd = (closest_other_block.actual_reward as f64
                            / 100_000_000.0)
                            * closest_other_block.btc_usd;

                        println!(
                            "{:<8} | {:<10} | {:<10.2} | {:<10.2} | {:<8} | {:<10} | {:<10.2} | {:<10.2}",
                            ocean_block.height,
                            ocean_block.timestamp,
                            ocean_block.loss_usd,
                            ocean_actual_usd,
                            closest_other_block.height,
                            closest_other_block.timestamp,
                            other_pool_actual_usd,
                            other_pool_estimated_loss_usd
                        );
                    }
                }
            }
        ...
        println!("{:->97}", "");
        println!(
            "TOTAL ESTIMATED CUMULATIVE LOSS for {}: ${:.2} ({} blocks compared)",
            other_pool_slug.to_uppercase(),
            estimated_other_pool_loss_usd,
            comparisons_made
        );
...
async fn main() -> Result<()> {
    ...
    if args.update {
        ...
        let mut historical_data = HistoricalPriceData {
            prices: price_lookup_map
                .iter()
                .map(|(&time, &usd)| PriceData { time, usd })
                .collect(),
        };
        ...
        if args.verbose {
            println!(
                "Full historical prices saved to: {}. Loaded {} entries.",
                output_file,
                price_lookup_map.len()
            );
        }
    } else {
        match std::fs::File::open(output_file) {
            Ok(file) => {
                ...
                price_lookup_map = historical_data
                    .prices
                    .into_iter()
                    .map(|p| (p.time, p.usd))
                    .collect();
                if args.verbose {
                    println!(
                        "DEBUG: Price data loaded from existing prices.json ({} entries).",
                        price_lookup_map.len()
                    );
                }
            }
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                if args.verbose {
                    println!(
                        "DEBUG: prices.json not found. Attempting to fetch full historical prices."
                    );
                }
    ...
    let other_pool_slugs: Vec<String> = if let Some(pools_str) = args.other_pools.as_ref() {
        ...
        if args.verbose {
            println!(
                "No --other-pools specified. Attempting to fetch active pools from mempool.space..."
            );
        }
        let fetched_pools = fetch_mempool_pools("1y", args.depth, &args).await?;
        if fetched_pools.is_empty() {
            eprintln!(
                "Error: Could not fetch any pools from mempool.space. Please specify --other-pools manually."
            );
            std::process::exit(1);
        } else {
            if args.verbose {
