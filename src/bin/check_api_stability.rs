...
async fn main() -> Result<()> {
    ...
    let sample1 = fetch_blocks_sample(5)
        .await
        .context("Failed to fetch sample 1")?;

    sleep(Duration::from_secs(2)).await;

    println!("Fetching sample 2...");
    let sample2 = fetch_blocks_sample(5)
        .await
        .context("Failed to fetch sample 2")?;
    ...
    for i in 0..sample1.len() {
        ...
        if match_rate1 != match_rate2 {
            println!(
                "Difference in matchRate for Block {} ({}): Sample 1={:?}, Sample 2={:?}",
        ...
        if reward1 != reward2 {
            println!(
                "Difference in reward for Block {} ({}): Sample 1={:?}, Sample 2={:?}",
        ...
        if expected_fees1 != expected_fees2 {
            println!(
                "Difference in expectedFees for Block {} ({}): Sample 1={:?}, Sample 2={:?}",
